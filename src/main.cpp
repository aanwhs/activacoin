...
#if defined(NDEBUG)
# error "Activacoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Activacoin Signed Message:\n";
...
{
    // Activacoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 250000); // Activacoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Activacoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Activacoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Activacoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Activacoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        assert(block.hashMerkleRoot == uint256("0x0bf282d218c191e430290c0f106db4c0ead67ea06e6da5c2e11edafa550d4efb"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc1, 0xb2, 0xdc }; // Activacoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ActivacoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ActivacoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ActivacoinMiner(CWallet *pwallet)
{
    printf("ActivacoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("activacoin-miner");
    ...
    try { loop {
        ...
        printf("Running ActivacoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ActivacoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ActivacoinMiner, pwallet));
}
